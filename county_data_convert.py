...
from crawler import dataConverter_daily_county
...
def get_daily_data():
    ...
    urllib.request.urlretrieve("https://api.covidactnow.org/v2/counties.csv?apiKey=8a4c770f9e9f4c8fb1ba252374ab4711","./daily/"+datetoday+"counties.csv")


def get_state_cbsas_data():
    urllib.request.urlretrieve("https://api.covidactnow.org/v2/counties.timeseries.csv?apiKey=8a4c770f9e9f4c8fb1ba252374ab4711","counties.timeseries.csv")
...
def convert_metro_daily():
    ...
    from datetime import date
    datetoday = date.today().strftime("%Y-%m-%d")
    df = pd.read_csv("./daily/"+datetoday+"counties.csv")
    df = df.values
    ...
    for i in fips_unique:
        sql = "select lat,lon, county,state from county_fips where fips = %s" % (str(i))
        cursor.execute(sql)
    ...
    print(data)
    data.to_csv("countiesdata.csv", index_label=False, index=False)
...
def convert_metro():
    ...
    print(type(fips_list))
    fips_unique = np.unique(fips_list)
    county_unique = np.unique(county_list)

    fips_actual = [] #without purto rico
    coord = []
    for i in range(len(fips_unique)):
        sql = "select lat,lon,county,state from county_fips where fips = %s" % (str(fips_unique[i]))
        # print(sql)
        # print(county_list[i])
        cursor.execute(sql)
        result = cursor.fetchall()
        if result:
            fips_actual.append(str(fips_unique[i]))
        #print(result)
            coord.append(list(result[0]))
        else:
            print(county_unique[i])
            # for k in range(len(fips_list)):
            #     if fips_unique[i] == fips_list[k]:
            #         #print(sql)
            #         #print(county_list[k])
            #         print(state_list[k])
            #         print("insert(["+str(fips_unique[i])+', "'+county_list[k]+'", "PR", , ])')
            #         break
    data = np.array(fips_actual).reshape(-1,1)
    ...
    for i in range(fips_list.shape[0]):
        #print(fips_list[i])
        location = np.argwhere(fips_unique == fips_list[i])
        #print(county_list[i],data.shape)
...
def convert_state():
    ...
    data = pd.DataFrame(data)
    print(data)
    data.to_csv("countystatedata.csv",index_label=False,index=False)


def sort_state():
    df = np.array(pd.read_csv("countiesdata.csv"))
    state_list = df[:, 4]
    #print(state_list)
    state_data = []
    ...
    for state in range(len(state_enum)):
        ...
        result.to_csv("./CountyState/%s.csv" % (state_enum[state]),index_label=False,index=False)


def daily_update_county():
    get_daily_data()
    get_state_cbsas_data()
    convert_state()
    today = convert_metro()
    sort_state()
    dataConverter_daily_county(today)
    return today
